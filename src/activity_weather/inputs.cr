module ActivityWeather
  ...
  struct Inputs
    DEFAULT_CONFIG_PATH    = ".github/activity-weather.yml"
    DEFAULT_COMMIT_MESSAGE = "chore: update activity weather"

    getter config_path : String
    # True when the config path was named — by input or flag — rather than
    # defaulted. A missing file is then an error instead of zero-config.
    getter? config_explicit : Bool
    getter workspace : String
    getter token : String?
    getter commit_message : String
    getter? commit : Bool
    # Conveniences for the zero-config case: point the action at a repo and
    # window without writing a config file. They override the config values.
    getter repo : String?
    getter period : String?

    def initialize(@config_path, @config_explicit, @workspace, @token,
                   @commit_message, @commit, @repo, @period)
    end

    def self.resolve(config_path : String? = nil, workspace : String? = nil,
                     commit_flag : Bool = false) : Inputs
      explicit = ENV["INPUT_CONFIG"]?.presence || config_path
      new(
        config_path: explicit || DEFAULT_CONFIG_PATH,
        config_explicit: !explicit.nil?,
        ...
        repo: ENV["INPUT_REPO"]?.presence,
        period: ENV["INPUT_PERIOD"]?.presence,
      )
    end
