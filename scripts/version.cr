...
module Version
  ...
  CANONICAL = "src/activity_weather/version.cr"
  ...
  SITES = [
    ...
    # survive appearing inside an image tag (`:v0.1.0`) and a ref (`@v0.1.0`).
    Site.new(
      path: "README.md",
      read: ->(body : String) { body.match(/^activity-weather v(\S+)$/m).try(&.[](1)) },
  ...
  # everywhere else while its notes were never written up.
