...
# Regenerate the README gallery. Needs network — activity comes from the API.
[group('documents')]
examples:
    #!/usr/bin/env bash
    set -euo pipefail
    [ -x bin/activity-weather ] || just build
    bin/activity-weather -c examples/showcase.yml
    for config in examples/variants/*.yml; do
        bin/activity-weather -c "$config"
    done

# Render a config the way the action would, without committing anything.
[group('documents')]
render config=".github/activity-weather.yml" repo="crystal-actions/activity-weather":
    ...
    docker run --rm \
        ...
        ghcr.io/crystal-actions/activity-weather:v0
