...
describe ActivityWeather::Concurrent do
  it "returns results in input order regardless of completion order" do
    items = [5, 1, 4, 2, 3]
    result = ActivityWeather::Concurrent.map(items, 4) do |item|
  ...
  it "keeps at most `limit` jobs in flight" do
    ...
    ActivityWeather::Concurrent.map((1..30).to_a, 4) do |item|
      tripwire.enter
      Fiber.yield
      tripwire.leave
      item
  ...
  it "runs every item even when limit exceeds the work" do
    seen = [] of Int32
    ActivityWeather::Concurrent.map([1, 2, 3], 99) { |item| seen << item; item }
  ...
  it "hands back an exception the block returned as an ordinary value" do
    ...
    result = ActivityWeather::Concurrent.map([1, 2], 2) do |item|
  ...
  it "re-raises the first failure in input order, not the first to happen" do
    ...
    error = expect_raises(Exception, "second") do
      ActivityWeather::Concurrent.map([1, 2, 3], 3) do |item|
        case item
  ...
  it "lets every worker finish before the failure surfaces" do
    ...
    expect_raises(Exception, "boom") do
      ActivityWeather::Concurrent.map((1..6).to_a, 3) do |item|
  ...
  it "raises straight through when there is nothing to overlap" do
    expect_raises(Exception, "alone") do
      ActivityWeather::Concurrent.map([1], 4) { |item| raise "alone" if item == 1; item }
    end
  end

  it "handles an empty work list" do
    ActivityWeather::Concurrent.map([] of Int32, 4) { |item| item }.should be_empty
  end
end
