...
describe "examples" do
  it "renders every committed config to a file the config names" do
    ...
    configs.each do |path|
      config = ActivityWeather::Config.load(path)
      ...
      paths.each do |output|
        ...
        File.exists?(repo_file(output)).should be_true,
          "#{relative} declares #{output}, which is missing — regenerate it with `bin/activity-weather -c #{relative}`"
      end
    end
  end
  ...
  it "links only to files that exist from the README" do
    ...
    prefix = "https://raw.githubusercontent.com/crystal-actions/activity-weather/main/"
  ...
  it "keeps the dogfood config loadable" do
    config = ActivityWeather::Config.load(repo_file(".github", "activity-weather.yml"))
    config.render_targets.size.should be > 0
  end
end
