...
describe ActivityWeather::HTTPPool do
  ...
  it "gives concurrent fibers their own connections and reuses each of them" do
    with_server do |server|
      pool = ActivityWeather::HTTPPool.new
      ActivityWeather::Concurrent.map((1..12).to_a, 4) do |index|
  ...
  it "gives up on a server that accepts and then says nothing" do
    ...
    with_server(mute: true) do |server|
      pool = ActivityWeather::HTTPPool.new(read_timeout: 150.milliseconds)
